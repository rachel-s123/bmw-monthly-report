...
import { Upload, Trash2, FileText, RefreshCw, Loader2, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
const FileUpload = ({ onDataUpdate, onError, onClearAll, onMetadataUpdate }) => {
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    // Reload all files from Supabase
  ...
  const removeFile = useCallback(async (index) => {
    try {
      ...
      if (file && file.name) {
        ...
        await loadExistingFiles(); // Reload files from Supabase
      }
    } catch (error) {
      console.error('Error removing file:', error);
      if (onError) {
        onError(`Failed to remove file: ${error.message}`);
      }
    }
  ...
  const clearAll = useCallback(async () => {
    try {
      ...
      if (onError) {
        onError(`Failed to clear all data: ${error.message}`);
      }
    }
  }, [onClearAll, onError]);
